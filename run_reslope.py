...
def dumpit():
    pickle.dump([(name, globals()[name]) for name in names if name in globals()], open('run_reslope.new.saved','w'))
...
# for name, res in pickle.load(open('run_reslope.new.saved')): globals()[name] = res
...
d = {}
for fname in glob.glob('reslope_out/*'):
    me, args, D = read_reslope_out(fname)
    l = len(D)
    if l == 0: continue
    D = np.array(D)
    X = D.mean(axis=0)
    ...
    #d[me][args]
#read_reslope_out('reslope_out/lols.1068')
...
"""
python run_reslope.py pos-tweet dagger::0.999 rmsprop 0.001 supervised embed=data/glove.6B.50d.txt.gz
...
"""
% python run_reslope.py --sweep 11949
...
Traceback (most recent call last):
  File "run_reslope.py", line 684, in <module>
    addl_args
  File "run_reslope.py", line 536, in run
    print_dots = False,
